...
<body  style="background-image: url('') !important;">
    ...
    <?php
        include 'config.php';
        $query = mysqli_query($mysqli, "SELECT * FROM tbl_pinjam WHERE peminjam='$username'");
        if(mysqli_num_rows($query) == 0){
            echo "<tr><td colspan='6'>belum ada data!</td></tr>";
        }else{
            $no = 1;
            while ($data = mysqli_fetch_array($query)) {
                $id_pinjam = $data['id']; // Menyimpan ID pinjaman
        ?>
                <tr>
                    <td><?php echo $no;?></td>
                    ...
                    <td>
                        <form action="proses-kembalikan.php?id=<?php echo $id_pinjam;?>" method="POST">
                            <input type="number" name="jml_kembali" min="1" max="<?php echo $data['jml_barang'];?>" required>
